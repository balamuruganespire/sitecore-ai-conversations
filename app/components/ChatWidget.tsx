  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "assistant",
      content:
        "Hi! Ask me anything about Sitecore — I'll answer only from your actual indexed content, with source citations.",
    },
  ]);
          ...
          setMessages((prev) => {
            ...
            if (c[c.length - 1]?.role === "assistant") {
              c[c.length - 1] = {
                ...c[c.length - 1],
                isStreaming: false,
                content: c[c.length - 1].content || "_(stopped)_",
              };
            }
            return c;
          });
      ...
      <div className="w-panel">
        ...
        <div className="w-header">
          <div className="w-header-left">
            ...
            <div>
              ...
              <div className="w-header-sub">
                <span
                  className={`w-status-dot ${streaming ? "pulsing" : ""}`}
                />
          ...
          <div className="w-header-actions">
            ...
            <label
              className="w-search-toggle"
              title={useSearch ? "Sitecore Search ON" : "Sitecore Search OFF"}
            >
              <input
                type="checkbox"
                checked={useSearch}
        ...
        <div className="w-tabs">
          <button
            className={`w-tab ${tab === "chat" ? "active" : ""}`}
            onClick={() => setTab("chat")}
          >
            <svg viewBox="0 0 16 16" fill="none">
              <path
                d="M14 10a1 1 0 01-1 1H5l-3 3V3a1 1 0 011-1h10a1 1 0 011 1v7z"
                fill="currentColor"
                opacity={tab === "chat" ? 1 : 0.35}
              />
            </svg>
            Chat
          </button>
          <button
            className={`w-tab ${tab === "search" ? "active" : ""}`}
          ...
          <div className="w-messages">
            {messages.map((msg, i) => (
              <div key={i} className={`w-msg ${msg.role}`}>
                {msg.role === "assistant" && (
                  <div
                    className={`w-msg-avatar ${msg.isStreaming ? "streaming" : ""}`}
                  >
                    <svg viewBox="0 0 16 16" fill="none">
                      <rect
                        x="1"
                  ...
                  {msg.role === "assistant" &&
                    ...
                    msg.feedback !== undefined && (
                      <div className="w-feedback">
                        <span className="w-feedback-label">
                          Was this helpful?
                        </span>
                        <button
                          className={`w-fb-btn ${msg.feedback === "up" ? "active" : ""}`}
                          onClick={() => setFeedback(i, "up")}
                        >
                          <svg viewBox="0 0 16 16" fill="none">
                            <path
                              d="M3 9h2V15H3zM5 9l3-6a2 2 0 012 2v2h3l-1 6H5V9z"
                              stroke="currentColor"
                              strokeWidth="1.1"
                              strokeLinejoin="round"
                              fill={
                                msg.feedback === "up" ? "currentColor" : "none"
                              }
                            />
                          </svg>
                        </button>
                        <button
                          className={`w-fb-btn ${msg.feedback === "down" ? "active down" : ""}`}
                          onClick={() => setFeedback(i, "down")}
                        >
                          <svg viewBox="0 0 16 16" fill="none">
                            <path
                              d="M13 7h-2V1h2zM11 7L8 13a2 2 0 01-2-2v-2H3l1-6h7v4z"
                              stroke="currentColor"
                              strokeWidth="1.1"
                              strokeLinejoin="round"
                              fill={
                        ...
                        {msg.feedback && (
                          <span className="w-fb-thanks">
                            {msg.feedback === "up"
                              ? "Thanks! 🎉"
                              : "Got it, we'll improve."}
                          </span>
                        )}
                      </div>
                    )}
        ...
        {tab === "chat" && messages.length === 1 && (
          <div className="w-starters">
            {STARTERS.map((s) => (
              <button key={s} className="w-starter" onClick={() => send(s)}>
                {s}
              </button>
            ))}
          </div>
          ...
          <div className="w-input-area">
            <div className="w-input-wrap">
              <textarea
                ...
                rows={1}
                placeholder={
                  streaming ? "Generating…" : "Ask a question or follow up…"
                }
                value={input}
                disabled={streaming}
              ...
              {streaming ? (
                ...
                <button
                  className={`w-send ${input.trim() ? "active" : ""}`}
                  onClick={() => send(input)}
                  disabled={!input.trim()}
                >
                  <svg viewBox="0 0 20 20" fill="none">
